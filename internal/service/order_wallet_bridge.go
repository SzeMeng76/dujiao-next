...
// applyOrderWalletBalance coordinates the order allocation update with the
...
func applyOrderWalletBalance(
	...
	amount, err := wallets.ApplyOrderBalance(walletgormstore.UseTransaction(tx), walletcontract.OrderBalanceInput{
		OrderID:          order.ID,
		UserID:           order.UserID,
	...
	if err := orders.WithTx(tx).UpdateFields(order.ID, map[string]interface{}{
...
func releaseOrderWalletBalance(
	...
	amount, err := wallets.ReleaseOrderBalance(
		walletgormstore.UseTransaction(tx),
		...
		func(now time.Time) (bool, error) {
			affected, updateErr := orders.WithTx(tx).UpdateFieldsWhereWalletPaid(order.ID, map[string]interface{}{
