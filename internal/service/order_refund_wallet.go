...
func (s *OrderRefundService) AdminRefundToWallet(
	input AdminRefundToWalletInput,
) (*models.Order, *walletdomain.Transaction, *models.OrderRefundRecord, error) {
	if input.OrderID == 0 {
	...
	err := s.orderRepo.Transaction(func(tx *gorm.DB) error {
		orderRepository := s.orderRepo.WithTx(tx)
		...
		} else if _, err := syncParentStatus(orderRepository, *order.ParentID, now); err != nil {
			return ErrOrderUpdateFailed
		}
