...
type ManagementService interface {
	GetUserManagementSnapshot(userID uint) (*models.ResellerProfile, []models.ResellerDomain, bool, error)
	ApplyUserReseller(userID uint, input resellermodule.ResellerApplyInput) (*models.ResellerProfile, error)
	SubmitUserCustomDomain(userID uint, rawDomain string) (*models.ResellerDomain, error)
}

type SiteConfigService interface {
	GetUserSiteConfig(userID uint) (*models.ResellerProfile, *models.ResellerSiteConfig, bool, error)
	UpdateUserSiteConfig(ctx context.Context, userID uint, input resellermodule.ResellerSiteConfigInput) (*models.ResellerSiteConfig, error)
}
...
func (h *UserHandler) ListDomains(c *gin.Context) {
	...
	if profile == nil {
		respondUserManagementError(c, resellermodule.ErrNotOpened, "error.user_fetch_failed")
		return
	}
