...
func (s managementStub) GetUserManagementSnapshot(userID uint) (*models.ResellerProfile, []models.ResellerDomain, bool, error) {
	return s.profile, s.domains, s.canApply, s.err
}

func (s managementStub) ApplyUserReseller(userID uint, input resellermodule.ResellerApplyInput) (*models.ResellerProfile, error) {
	if s.err != nil {
		return nil, s.err
	}
	if s.profile != nil {
	...
	return &models.ResellerProfile{UserID: userID, Status: models.ResellerProfileStatusPendingReview, ApplyReason: input.Reason}, nil
}

func (s managementStub) SubmitUserCustomDomain(userID uint, rawDomain string) (*models.ResellerDomain, error) {
	if s.err != nil {
		return nil, s.err
	}
	return &models.ResellerDomain{Domain: rawDomain}, nil
}
...
func (s siteConfigStub) GetUserSiteConfig(userID uint) (*models.ResellerProfile, *models.ResellerSiteConfig, bool, error) {
	return s.profile, s.row, s.canEdit, s.err
}

func (s siteConfigStub) UpdateUserSiteConfig(ctx context.Context, userID uint, input resellermodule.ResellerSiteConfigInput) (*models.ResellerSiteConfig, error) {
	if s.err != nil {
		return nil, s.err
	}
	return &models.ResellerSiteConfig{SiteName: input.SiteName, Logo: input.Logo}, nil
}
...
func TestUserHandlerApplyAndSnapshot(t *testing.T) {
	...
	profile := &models.ResellerProfile{ID: 9, UserID: 7, Status: models.ResellerProfileStatusPendingReview}
...
func TestUserHandlerSubmitCustomDomainMapsInactiveProfile(t *testing.T) {
	gin.SetMode(gin.TestMode)
	h := NewUserHandler(managementStub{err: resellermodule.ErrProfileInactive}, siteConfigStub{}, uploadStub{})
...
func TestUserHandlerSiteConfigUpdateAndGet(t *testing.T) {
	gin.SetMode(gin.TestMode)
	profile := &models.ResellerProfile{ID: 3, UserID: 8, Status: models.ResellerProfileStatusActive}
	h := NewUserHandler(
		managementStub{profile: profile},
		siteConfigStub{profile: profile, canEdit: true, row: &models.ResellerSiteConfig{SiteName: "Alice Store"}},
		uploadStub{},
	)
