...
func setupAdminContentHandlerTest(t *testing.T) (*contenttransport.AdminHandler, *domaincontent.PostService, *gorm.DB) {
	t.Helper()
	...
	postService := domaincontent.NewPostService(
		postStore,
		postStore,
	...
	handler := contenttransport.NewAdminHandler(
		postService,
		domaincontent.NewPostCategoryService(gormstore.NewPostCategoryStore(db)),
		domaincontent.NewBannerService(gormstore.NewBannerStore(db), domaincontent.SystemClock{}),
		domaincontent.NewMediaService(gormstore.NewMediaStore(db), localfilestore.New(), nil),
	)
...
func TestAdminContentHandlersSuccessContracts(t *testing.T) {
	...
	var inactiveBanner models.Banner
...
func TestAdminContentHandlersValidationAndDomainErrorContracts(t *testing.T) {
	...
	if _, err := posts.Create(context.Background(), domaincontent.CreatePostInput{
		Slug:      "duplicate",
		Type:      "blog",
