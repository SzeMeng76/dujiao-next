...
func setupPublicContentHandlerTest(t *testing.T) (
	...
	*domaincontent.PostService,
	*domaincontent.PostCategoryService,
	*domaincontent.BannerService,
	*gorm.DB,
) {
	t.Helper()
	...
	postCategoryService := domaincontent.NewPostCategoryService(gormstore.NewPostCategoryStore(db))
	bannerService := domaincontent.NewBannerService(gormstore.NewBannerStore(db), domaincontent.SystemClock{})
...
func TestPublicContentHandlersReturnOnlyPublicData(t *testing.T) {
	...
	if _, err := banners.Create(context.Background(), domaincontent.BannerInput{
		Name:     "public banner",
