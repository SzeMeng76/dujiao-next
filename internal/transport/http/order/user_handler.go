...
type UserOrderQuery interface {
	ListOrdersByUserForTenant(tenant reseller.TenantContext, filter UserOrderListFilter) ([]models.Order, int64, error)
	...
	GetAnyOrderByUserOrderNoForTenant(tenant reseller.TenantContext, orderNo string, userID uint) (*models.Order, error)
	CancelOrder(orderID uint, userID uint) (*models.Order, error)
}
...
type PaymentChannelPolicy interface {
	GetAllowedChannelIDsForOrder(items []models.OrderItem) []uint
...
func enrichOrderWithAllowedChannels(payments PaymentChannelPolicy, order *models.Order, detail *dto.OrderDetail) {
...
func enrichOrderWithRefundRecords(refunds RefundRecordDirectory, order *models.Order, detail *dto.OrderDetail) {
