...
type OrderCreateService interface {
	...
	CreateGuestOrder(input CreateGuestOrderInput) (*models.Order, error)
}
...
func (h *CreateHandler) respondCreateAndPay(c *gin.Context, order *models.Order, orderResp dto.OrderDetail, channelID uint, useBalance bool) {
	if h.pay == nil {
		response.Success(c, gin.H{
			"order":         orderResp,
			"order_no":      order.OrderNo,
