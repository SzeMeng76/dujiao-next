...
func (f *fakeAdminPaymentQuery) ListPayments(filter AdminPaymentListFilter) ([]models.Payment, int64, error) {
	out := make([]models.Payment, 0, len(f.payments))
...
func (f *fakeAdminPaymentQuery) GetPayment(id uint) (*models.Payment, error) {
...
func (f fakeAdminChannelLookup) ListByIDs(ids []uint) ([]models.PaymentChannel, error) {
	out := make([]models.PaymentChannel, 0, len(ids))
	for _, id := range ids {
		out = append(out, models.PaymentChannel{ID: id, Name: f.names[id]})
	}
	return out, nil
}
