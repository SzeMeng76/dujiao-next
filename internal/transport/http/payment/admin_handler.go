...
type AdminPaymentQuery interface {
	ListPayments(filter AdminPaymentListFilter) ([]models.Payment, int64, error)
...
func (h *AdminHandler) writeAdminPaymentCSVRows(writer *csv.Writer, payments []models.Payment) error {
	rechargeMetaMap, err := h.resolvePaymentRechargeMeta(payments)
	if err != nil {
		return err
	}
...
func (h *AdminHandler) resolvePaymentOrderNos(payments []models.Payment) (map[uint]string, error) {
...
func (h *AdminHandler) resolvePaymentRechargeMeta(payments []models.Payment) (map[uint]paymentRechargeMeta, error) {
