...
type WalletRechargeOrder struct {
	...
	Amount          money.Amount   `gorm:"type:decimal(20,2);not null" json:"amount"`                // 充值金额
	PayableAmount   money.Amount   `gorm:"type:decimal(20,2);not null" json:"payable_amount"`        // 实际支付金额（含手续费）
	FeeRate         money.Amount   `gorm:"type:decimal(6,2);not null;default:0" json:"fee_rate"`     // 手续费比例
