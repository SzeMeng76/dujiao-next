...
func setupPromotionRepositoryTest(t *testing.T) *gormstore.Store {
	t.Helper()
...
func createTestPromotion(t *testing.T, repo *gormstore.Store, name string) {
	t.Helper()
...
func TestPromotionRepositoryListFilterByName(t *testing.T) {
	...
	rows, total, err := repo.List(promotion.ListFilter{Name: "双十一"})
	if err != nil {
...
func TestPromotionRepositoryListFilterByNameCaseInsensitive(t *testing.T) {
	...
	rows, total, err := repo.List(promotion.ListFilter{Name: "year sale"})
	if err != nil {
		t.Fatalf("list promotions failed: %v", err)
	}
	if total != 1 || len(rows) != 1 {
...
func TestPromotionRepositoryListEmptyNameReturnsAll(t *testing.T) {
	...
	rows, total, err := repo.List(promotion.ListFilter{Name: ""})
	if err != nil {
		t.Fatalf("list promotions failed: %v", err)
	}
