...
func (ctx *OrderPricingContext) BuildSnapshot(orderID uint, now time.Time) *models.ResellerOrderSnapshot {
	if ctx == nil {
		return nil
	}
...
func BuildSettingIndexes(settings []models.ResellerProductSetting) (map[uint]*models.ResellerProductSetting, map[SettingKey]*models.ResellerProductSetting) {
	byProduct := make(map[uint]*models.ResellerProductSetting)
	bySKU := make(map[SettingKey]*models.ResellerProductSetting)
...
func ApplySelfDealingRisk(ctx *OrderPricingContext, profile *models.ResellerProfile, relatedAccountMatch bool) {
