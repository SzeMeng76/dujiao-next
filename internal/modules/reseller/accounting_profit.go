...
type AccountingLedgerService struct {
	store       AccountingLedgerStore
	confirmDays int
}

func NewAccountingLedgerService(store AccountingLedgerStore, confirmDays int) *AccountingLedgerService {
...
func (s *AccountingLedgerService) PostOrderProfit(store AccountingLedgerStore, order *models.Order, payment *models.Payment) error {
	...
	if entry.Currency == "" {
		return ErrLedgerInvalidSnapshot
	}
...
func (s *AccountingLedgerService) ConfirmDueLedgerEntries(now time.Time) (int64, error) {
	...
	var affected int64
	err := s.store.WithinTransaction(func(store AccountingLedgerStore) error {
