...
func DecodeDashboardSetting(raw models.JSON, fallback DashboardSetting) DashboardSetting {
	...
	if alert, ok := raw["alert"].(map[string]interface{}); ok {
		...
		if parsed, err := parseInt(alert["out_of_stock_products_threshold"]); err == nil {
			result.Alert.OutOfStockProductsThreshold = int64(parsed)
		}
		if parsed, err := parseInt(alert["pending_payment_orders_threshold"]); err == nil {
			result.Alert.PendingPaymentOrdersThreshold = int64(parsed)
		}
		if parsed, err := parseInt(alert["payments_failed_threshold"]); err == nil {
			result.Alert.PaymentsFailedThreshold = int64(parsed)
		}
	}
	if ranking, ok := raw["ranking"].(map[string]interface{}); ok {
		if parsed, err := parseInt(ranking["top_products_limit"]); err == nil {
			result.Ranking.TopProductsLimit = parsed
		}
		if parsed, err := parseInt(ranking["top_channels_limit"]); err == nil {
