...
func DecodeUpstreamSyncConfig(raw jsonmap.JSON, fallback UpstreamSyncConfig) UpstreamSyncConfig {
	...
	if value, exists := raw[constants.SettingFieldUpstreamPreOrderCheck]; exists {
		result.PreOrderStockCheckEnabled = parseBool(value)
	}
	if parsed, err := parseInt(raw[constants.SettingFieldUpstreamSyncPageSize]); err == nil {
		result.SyncPageSize = parsed
	}
	if parsed, err := parseInt(raw[constants.SettingFieldUpstreamSyncMaxPages]); err == nil {
		result.SyncMaxPages = parsed
	}
	if parsed, err := parseInt(raw[constants.SettingFieldUpstreamSyncConcurrency]); err == nil {
