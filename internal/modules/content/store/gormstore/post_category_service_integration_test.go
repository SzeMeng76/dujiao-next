...
func newPostCategoryServiceForTest(t *testing.T) (*domaincontent.PostCategoryService, *gorm.DB) {
	t.Helper()
	...
	if err := db.AutoMigrate(&models.PostCategory{}, &models.Post{}); err != nil {
		t.Fatalf("auto migrate post category tables: %v", err)
	}

	return domaincontent.NewPostCategoryService(gormstore.NewPostCategoryStore(db)), db
}

func createCategoryThroughService(t *testing.T, svc *domaincontent.PostCategoryService, slug string, parentID *uint, sortOrder int) *models.PostCategory {
	t.Helper()
...
func TestPostCategoryServiceCreateRejectsInvalidParentAndDuplicateSlug(t *testing.T) {
	...
	for name, parentID := range map[string]*uint{
		...
		t.Run(name, func(t *testing.T) {
			...
			if err != domaincontent.ErrCategoryParentInvalid {
...
func TestPostCategoryServiceDeleteRejectsCategoriesInUse(t *testing.T) {
	...
	if err := svc.Delete(context.Background(), root.ID); err != domaincontent.ErrCategoryInUse {
	...
	if err := svc.Delete(context.Background(), withPost.ID); err != domaincontent.ErrCategoryInUse {
	...
	if _, err := svc.GetByID(context.Background(), empty.ID); err != domaincontent.ErrNotFound {
