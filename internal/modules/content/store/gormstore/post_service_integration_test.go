...
func newPostServiceForTest(t *testing.T) (*domaincontent.PostService, *gorm.DB) {
	t.Helper()
	...
	return domaincontent.NewPostService(
		postStore,
		postStore,
		gormstore.NewPostCategoryStore(db),
		domaincontent.SystemClock{},
	), db
}
...
func TestPostServiceCreateRejectsNoticeCategory(t *testing.T) {
	...
	_, err := svc.Create(context.Background(), domaincontent.CreatePostInput{
		Slug:       "notice-with-category",
		Type:       constants.PostTypeNotice,
		TitleJSON:  map[string]interface{}{"zh-CN": "notice-with-category"},
		CategoryID: &leaf.ID,
	})
	if err != domaincontent.ErrPostNoticeCategoryUnsupported {
...
func TestPostServiceCreateRejectsMissingOrParentCategory(t *testing.T) {
	...
	if err != domaincontent.ErrPostCategoryInvalid {
...
func TestPostServiceUpdateRejectsUnsupportedOrInvalidCategoryAssignment(t *testing.T) {
	...
	_, err := svc.Update(context.Background(), fmt.Sprintf("%d", post.ID), domaincontent.CreatePostInput{
		Slug:       post.Slug,
		Type:       constants.PostTypeNotice,
...
func TestPostServiceCategoryAssignmentRespectsInactive(t *testing.T) {
	...
	updated, err := svc.Update(context.Background(), fmt.Sprintf("%d", post.ID), domaincontent.CreatePostInput{
		Slug:       post.Slug,
		Type:       constants.PostTypeBlog,
		TitleJSON:  map[string]interface{}{"zh-CN": post.Slug},
		CategoryID: &inactive.ID,
...
func TestPostServiceListPublicFiltersDraftsAndOrdersByPublishedAt(t *testing.T) {
	...
	posts, total, err := svc.ListPublic(context.Background(), domaincontent.PublicPostQuery{
		Type:     constants.PostTypeBlog,
		Page:     1,
		PageSize: 20,
	})
...
func TestPostServiceGetPublicBySlugHidesDraftsAndMissingPosts(t *testing.T) {
	...
	if _, err := svc.GetPublicBySlug(context.Background(), "draft-post"); err != domaincontent.ErrNotFound {
		t.Fatalf("expected draft to be hidden as not found, got %v", err)
	}
	if _, err := svc.GetPublicBySlug(context.Background(), "missing-post"); err != domaincontent.ErrNotFound {
...
func TestPostServicePublishedAtIsSetOnlyOnFirstPublish(t *testing.T) {
	...
	draft, err := svc.Create(context.Background(), domaincontent.CreatePostInput{
	...
	published := true
	firstPublish, err := svc.Update(context.Background(), fmt.Sprintf("%d", draft.ID), domaincontent.CreatePostInput{
		Slug:        draft.Slug,
		Type:        constants.PostTypeBlog,
	...
	published = false
	if _, err := svc.Update(context.Background(), fmt.Sprintf("%d", draft.ID), domaincontent.CreatePostInput{
		Slug:        draft.Slug,
		Type:        constants.PostTypeBlog,
	...
	published = true
	republished, err := svc.Update(context.Background(), fmt.Sprintf("%d", draft.ID), domaincontent.CreatePostInput{
		Slug:        draft.Slug,
		Type:        constants.PostTypeBlog,
...
func TestPostServiceRelatedProductIDsPreserveOrderAndDeduplicate(t *testing.T) {
	...
	post, err := svc.Create(context.Background(), domaincontent.CreatePostInput{
...
func TestPostServiceCreateRollsBackPostWhenRelatedProductsFail(t *testing.T) {
	...
	var postCount int64
	if countErr := db.Model(&models.Post{}).Where("slug = ?", "atomic-create").Count(&postCount).Error; countErr != nil {
...
func TestPostServiceUpdateRollsBackPostAndRelationsWhenRelatedProductsFail(t *testing.T) {
	...
	if err := db.Create(&models.PostProduct{PostID: post.ID, ProductID: 7}).Error; err != nil {
	...
	if relationErr := db.Model(&models.PostProduct{}).
...
func TestPostServiceListPostsForProductFiltersTypeAndPublication(t *testing.T) {
	...
	create := func(slug, postType string, isPublished *bool) {
		t.Helper()
		if _, err := svc.Create(context.Background(), domaincontent.CreatePostInput{
			Slug:        slug,
			Type:        postType,
