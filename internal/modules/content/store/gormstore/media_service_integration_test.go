...
func setupMediaServiceTest(t *testing.T) (*domaincontent.MediaService, *gorm.DB) {
	t.Helper()
	...
	return domaincontent.NewMediaService(
...
func TestMediaServiceRenameTrimsAndValidatesName(t *testing.T) {
	...
	if err := svc.Rename(context.Background(), media.ID, "   "); err != domaincontent.ErrMediaNameEmpty {
		t.Fatalf("blank name should return domaincontent.ErrMediaNameEmpty, got %v", err)
	}
	if err := svc.Rename(context.Background(), 9999, "missing"); err != domaincontent.ErrMediaNotFound {
...
func TestMediaServiceDeleteKeepsBestEffortFileFailureSemantics(t *testing.T) {
	...
	var visible int64
	if err := db.Model(&models.Media{}).Where("id = ?", media.ID).Count(&visible).Error; err != nil {
	...
	var unscoped int64
	if err := db.Unscoped().Model(&models.Media{}).Where("id = ?", media.ID).Count(&unscoped).Error; err != nil {
