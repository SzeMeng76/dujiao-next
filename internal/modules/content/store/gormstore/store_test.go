...
func TestPostStoreQueriesAndOrderedRelations(t *testing.T) {
	...
	listed, total, err := store.List(ctx, content.PostQuery{
		Page:          1,
		PageSize:      20,
...
func TestBannerStoreSearchAndValidityWindow(t *testing.T) {
	...
	if err := db.Model(&models.Banner{}).Where("id = ?", banners[3].ID).Update("is_active", false).Error; err != nil {
		t.Fatalf("disable banner fixture: %v", err)
	}

	listed, total, err := store.List(ctx, content.BannerQuery{Page: 1, PageSize: 20, Search: "hero"})
	if err != nil {
...
func TestMediaStoreFiltersUpdatesAndSoftDeletes(t *testing.T) {
	...
	listed, total, err := store.List(ctx, content.MediaQuery{Page: 1, PageSize: 20, Scene: "common", Search: "special"})
	if err != nil {
...
func TestStoresPropagateCancelledContext(t *testing.T) {
	...
	_, _, err := NewPostStore(db).List(ctx, content.PostQuery{Page: 1, PageSize: 20})
	if err == nil {
