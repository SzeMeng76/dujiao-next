...
// RefRepository 是回调服务所需的下游引用持久化端口。
...
type CallbackQueue interface {
	EnqueueDownstreamCallback(payload queue.DownstreamCallbackPayload, opts ...asynq.Option) error
}

// Service 是 B 侧下游回调通知服务。
type Service struct {
	refRepo        RefRepository
	orderRepo      OrderReader
	credentialRepo CredentialReader
	queueClient    CallbackQueue
...
// CreateRef 创建下游订单引用
func (s *Service) CreateRef(ref *models.DownstreamOrderRef) error {
...
func (s *Service) GetByOrderID(orderID uint) (*models.DownstreamOrderRef, error) {
...
// SendCallback 执行回调发送
func (s *Service) SendCallback(refID uint) error {
...
func (s *Service) handleCallbackFailure(ref *models.DownstreamOrderRef, now *time.Time, callbackErr error) error {
