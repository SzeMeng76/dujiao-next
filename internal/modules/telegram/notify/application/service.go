...
func (s *Service) SendMessage(ctx context.Context, chatID, message string) error {
	return s.SendMessageWithOptions(ctx, notifycontract.SendOptions{
		ChatID:                chatID,
		Message:               message,
...
// SendMessageWithOptions 使用当前配置的 Bot Token 发送消息（支持 inline 按钮等附加项）。
func (s *Service) SendMessageWithOptions(ctx context.Context, options notifycontract.SendOptions) error {
	...
	return s.sender.SendWithBotToken(ctx, token, options)
}
